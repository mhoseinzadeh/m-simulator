 ...
 */
const char* Parser::parse(const char new_expr[])
{
    try
	{
    	user_var.add("inf", -1);
...
//        user_var.add("Ans", ans);
		if(ans > 1000 || ans < -1000) {
			...
			sprintf(ans_str, "%g", (double)ans);
		}
    }
    catch (Error err)
    {
		sprintf(ans_str, "%s", err.get_msg());
	}
    
    return ((string)ans_str).c_str();
}

bool Parser::var_exist(const char* name) {
    return this->user_var.exist(name);
}
...
int Parser::get_operator_id(const char op_name[])
{
	...
	if (!strcmp(op_name, "&&")) {return AND;}
	if (!strcmp(op_name, "||")) {return OR;}
	if (!strcmp(op_name, "and")) {return AND;}
	if (!strcmp(op_name, "or")) {return OR;}
	...
	if (!strcmp(op_name, "xor")) {return XOR;}
...
double Parser::eval_function(const char fn_name[], const double &value)
{
    ...
    try
    {
		...
		if (!strcmp(fnU, "SQRT")) {if(value<0) domain_error=true; else return sqrt(value);}
		if (!strcmp(fnU, "LN")) {if(value<0) domain_error=true; else return log(value);}
		if (!strcmp(fnU, "LOG")) {if(value<0) domain_error=true; else return log(value);}
		if (!strcmp(fnU, "LOG2")) {if(value<0) domain_error=true; else return log(value)/log(2.0);}
		if (!strcmp(fnU, "LOG10")) {if(value<0) domain_error=true; else return log10(value);}

		// trigonometric
		if (!strcmp(fnU, "SIN")) {return sin(value);}
		if (!strcmp(fnU, "COS")) {return cos(value);}
		if (!strcmp(fnU, "TAN")) {return tan(value);}
		if (!strcmp(fnU, "ASIN")) {if(value<0) domain_error=true; else return asin(value);}
		if (!strcmp(fnU, "ACOS")) {if(value<0) domain_error=true; else return acos(value);}
		if (!strcmp(fnU, "ATAN")) {return atan(value);}
		if (!strcmp(fnU, "CEIL")) {return ceil(value);}
		if (!strcmp(fnU, "FLOOR")) {return floor(value);}
